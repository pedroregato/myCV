...
O mesmo método aparece no CIDA (96,82% de acurácia na classificação de acervo acadêmico, em produção no HPC da FGV) e no DataJud Monitor (plataforma própria de monitoramento processual multi-tribunal). Problema encomendado ou problema que eu mesmo enxerguei - a disciplina é a mesma: medir antes de desenhar, integrar Modelo -> Sistema -> Processo, construir governança embutida, entregar e operar sozinho.
